...
import com.readboy.localsvgmap.Dom2XmlUtils.dom2xml
...
class MapView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    ...
    private var minWidth = dp2px(getContext(), 800f)
    private var minHeight = dp2px(getContext(), 640f)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        when (widthMode) {
            ...
            MeasureSpec.AT_MOST -> {
                ...
                width = width.coerceAtMost(minWidth.toInt())
            }
            else -> {
        ...
        scale
        //计算需要缩放的比例
        scale = width*1.0f / svgWidth
        scale = (height*1.0f / svgHeight).coerceAtMost(scale)
        setMeasuredDimension(width, height)
        Log.d("", ""+ width+" , "+height+"=2=divhee===============svgWidth=="+svgWidth+ " , " + svgHeight+ "==="+scale);
    }
